...
// @updateURL    https://blhx.danmu9.com/blhxfy/extension.ios.user.js
...
(function () {
	...
	const getLocalConfig = () => {
	  ...
	  const keys = ['autoDownload', 'bottomToolbar', 'displayName', 'removeScroller', 'hideSidebar', 'originText', 'storyOnly', 'showTranslator', 'transJa', 'transEn', 'keepBgm', 'font', 'transApi', 'fontBold', 'plainText', 'battleTrans', 'log', 'defaultFont'];
	  keys.forEach(key => {
	...
	const request = async pathname => {
	  {
	    return new Promise((rev, rej) => {
	      fetch(`${config.origin}${pathname}`).then(res => {
	        if (!res.ok) {
	...
	*/
	...
	const getScenario = async name => {
	  ...
	  if (!csv) {
	    ...
	    csv = await fetchWithHash(`/blhxfy/data/story/${pathname}`);
	  }
	...
	const skillKeys = [['special_skill', 'special'], ['action_ability1', 'skill-1'], ['action_ability2', 'skill-2'], ['action_ability3', 'skill-3'], ['action_ability4', 'skill-4'], ['support_ability1', 'support-1'], ['support_ability2', 'support-2'], ['support_ability_of_npczenith', 'skill-lb'], ['appear_ability', 'skill-main'], ['backmember_ability1', 'skill-sub']];
	const state = {
	  status: 'init',
	  cStatus: 'init',
	  locSkMap: false,
	...
	const getBattleNoteQuest = async id => {
	  ...
	  if (questNote[id]) {
	    ...
	    if (!questNoteData || !questNoteData[id]) {
	      let csv = await fetchWithHash(`/blhxfy/data/battle/note/${questNote[id]}`);
	      questNoteData = {
	        [id]: csv
	      };
